function [indx,error_data]=find_ideal_sinter_convex(data)
%% 变量说明
% 输出变量----------------------------------------------------------------
...
[zb0,indx]=deal(zeros(size(data,1),1));
...
%     end
    ...
    if  con_2 & con_3 & con_4 & con_5
        indx(i)=5;
        error_data{i,1}=[];
    end
    % 不满足上凸约束
    if  con_2 & con_3 & (con_4==0 | con_5==0)
        indx(i)=4;
        error_data{i,1}=find(datadf_2(20:25)>0)+20;
    end
    % 左右两侧均不满足单增单减约束
    if   (con_2==0) & (con_3==0)
        indx(i)=3;
